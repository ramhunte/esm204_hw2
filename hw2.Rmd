...
As seen in the demand curves for each school, UCD is willing to pay the most for the first ton of carbon at about $484.
...
**UCSB**
UCSB will emit  40209.19  and abate 49790.81 tons of carbon when the price is $50 per ton of carbon.

**UCLA**
UCLA will emit  178717.5  and abate 71282.47 tons of carbon when the price is $50 per ton of carbon.

**UCI**
UCI will emit 75096.24 and abate 24903.76 tons of carbon when the price is $50 per ton of carbon.

**UCD**
UCD will emit 143466.4 and abate 16533.62 tons of carbon when the price is $50 per ton of carbon.


*What is the total amount of emissions across all four campuses? How much tax revenue is generated?*

All of the UC's combined would generate 437489.3 tons of carbon if each ton of carbon were taxed at $50. This translates into 
...
ucla_abate_func( q= 150000)

#uci
uci_abate_func(q= 0)

#ucd
ucd_abate_func( q= 60000)
...
trade_data <- data.frame(price = seq(0,500, length.out=101)) %>% 
  ...
  mutate(Q_ucsb = pmax(0, find_q(a= ucsb_dem_lm$coefficients[[1]], p = trade_data$price, b=ucsb_dem_lm$coefficients[[2]] ))) %>% 
  mutate(Q_ucla = pmax(0, find_q(a= ucla_dem_lm$coefficients[[1]], p = trade_data$price, b=ucla_dem_lm$coefficients[[2]] ))) %>% 
  mutate(Q_uci = pmax(0, find_q(a= uci_dem_lm$coefficients[[1]], p = trade_data$price, b=uci_dem_lm$coefficients[[2]] ))) %>% 
  mutate(Q_ucd = pmax(0, find_q(a= ucd_dem_lm$coefficients[[1]], p = trade_data$price, b=ucd_dem_lm$coefficients[[2]] ))) %>% 
...
#How much would eahc campus emit? 
...
The equilibrium price (p star) for the aggregate demand of pollution for all combined universities capped at 400,000 tons of carbon is **$61.53459**. 
...
redo question 3d with 350000 instead of 400000
